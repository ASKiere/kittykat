...
void txtRead()
{
	...
	if(home == NULL)
	{
		printf("Kittykat is made for linux!");
		return;
	}
	
	char kittykatconfig[256];
	snprintf(kittykatconfig, sizeof(kittykatconfig), "%s/kitty/kittykat.txt", home);

	FILE* txt = fopen(kittykatconfig,"r");
	if(txt == NULL)
	{
